...
export function createSudoku(input) {
    ...
    // 获取下一步提示，先找裸单（naked single），再找隐藏单（hidden single）
    function getNextHint() {
        // 1. Naked single：单元格只剩一个候选数
        for (let r = 0; r < SUDOKU_SIZE; r++) {
            for (let c = 0; c < SUDOKU_SIZE; c++) {
                if (grid[r][c] === 0) {
        ...
        // 2. Hidden single：在某行/列/宫中，某个数字只能填在一个格子里
        for (let n = 1; n <= 9; n++) {
            // 按行扫描
            for (let r = 0; r < SUDOKU_SIZE; r++) {
                let possible = [];
                for (let c = 0; c < SUDOKU_SIZE; c++) {
                    if (grid[r][c] === 0) {
                        const candidates = getCandidates(r, c);
                        if (candidates.includes(n)) possible.push(c);
                    }
                }
                if (possible.length === 1) {
                    return { row: r, col: possible[0], value: n };
                }
            }
            // 按列扫描
            for (let c = 0; c < SUDOKU_SIZE; c++) {
                ...
                for (let r = 0; r < SUDOKU_SIZE; r++) {
                    if (grid[r][c] === 0) {
                        const candidates = getCandidates(r, c);
                        if (candidates.includes(n)) possible.push(r);
                    }
                }
                if (possible.length === 1) {
                    return { row: possible[0], col: c, value: n };
                }
            }
            // 按宫扫描
            for (let br = 0; br < SUDOKU_SIZE; br += BOX_SIZE) {
                for (let bc = 0; bc < SUDOKU_SIZE; bc += BOX_SIZE) {
                    let possible = [];
                    for (let r = br; r < br + BOX_SIZE; r++) {
                        for (let c = bc; c < bc + BOX_SIZE; c++) {
                            if (grid[r][c] === 0) {
                                const candidates = getCandidates(r, c);
                                if (candidates.includes(n)) possible.push({ row: r, col: c });
                            }
                        }
                    }
                    if (possible.length === 1) {
                        return { row: possible[0].row, col: possible[0].col, value: n };
                    }
                }
            }
        }
